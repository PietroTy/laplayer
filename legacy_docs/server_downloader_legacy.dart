...
class ServerDownloader {
  Future<File?> downloadTrack({
    ...
    try {
      ...
      final accessKey = prefs.getString('access_key')?.trim() ?? '';
      ...
      try {
        response = await dio.download(
          ...
          options: Options(
            ...
            headers: {
              if (accessKey.isNotEmpty) 'X-Access-Key': accessKey,
            },
          ),
      ...
      final folderName = await _getPlaylistFolderName(playlistId);
      final musicDir = Directory(p.join(musicDirRoot, folderName));
  ...
  Future<String> _getPlaylistFolderName(String playlistId) async {
    try {
      ...
      if (rows.isNotEmpty) {
        ...
        if (name.isNotEmpty) {
          final sanitize = (String s) => s.replaceAll(RegExp(r'[<>:"/\\|?*]'), '').trim();
          final folderName = sanitize(name);
