...
class HomeScreen extends ConsumerWidget {
            ...
            actions: [
              ...
              IconButton(
                icon: sync.isLoading
                    ...
                    : const Icon(Icons.sync_rounded, color: AppColors.textSecond),
                onPressed: sync.isLoading
                    ? null
                    : () {
                        final empty = playlists.value?.isEmpty ?? true;
                        if (empty) {
                          context.push('/add-playlist');
                        } else {
          ...
          if (sync.isLoading || (sync.message != null && sync.message!.isNotEmpty))
            SliverToBoxAdapter(
              child: Container(
                ...
                child: Text(
                  sync.message ?? (sync.isLoading ? 'Sincronizando...' : ''),
                          ...
                          children: [
                            ...
                            if (data.mostPlayed.isEmpty)
                              ...
                              ...List.generate(data.mostPlayed.length, (index) {
                                ...
                                return _buildRankingTile(index + 1, track, playCount, ref);
                              }),
                          ],
                        ),
                      ),
  ...
  Widget _buildRankingTile(int rank, Track track, int plays, WidgetRef ref) {
    return InkWell(
      onTap: () {
        ref.read(playerProvider.notifier).playQueue([track], startIndex: 0);
      },
      borderRadius: BorderRadius.circular(12),
      child: Padding(
