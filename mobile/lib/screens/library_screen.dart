...
class _PlaylistTile extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return ListTile(
      ...
      trailing: Row(
        ...
        children: [
          if (!playlist.syncDisabled)
            IconButton(
              icon: const Icon(Icons.sync_rounded, color: AppColors.textSecond, size: 22),
              tooltip: 'Sincronizar Playlist',
              onPressed: () => _showSyncConfirm(context, ref),
            ),
          ...
          builder: (context) => SafeArea(
            child: Column(
              ...
              children: [
                ...
                if (!playlist.syncDisabled)
                  ListTile(
                    leading: Icon(Icons.sync_rounded, color: AppColors.accent),
                    title: const Text('Sincronizar Playlist', style: TextStyle(color: AppColors.textPrimary)),
                    subtitle: const Text('Busca alterações diretamente no Spotify'),
                    onTap: () {
                      Navigator.pop(context);
                      _showSyncConfirm(context, ref);
                    },
                  ),
                ListTile(
  ...
  void _showSyncConfirm(BuildContext context, WidgetRef ref) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        ...
        content: Text(
          'Esta ação irá consultar o Spotify para sincronizar e atualizar as faixas de "${playlist.name}".\n\n'
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () {
              Navigator.pop(context);
              ref.read(syncProvider.notifier).syncMetadata(playlist.id);
            },
